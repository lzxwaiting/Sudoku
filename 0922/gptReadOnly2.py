...
def create_game_interface():
        ...
        for i in range(9):
            for j in range(9):
                if sudoku_puzzles[idx][i][j] == 0:
                    ...
                    entries[idx][i][j].configure(bg='gray', state='normal')
                else:
                    entries[idx][i][j].delete(0, tk.END)
                    entries[idx][i][j].insert(0, str(sudoku_puzzles[idx][i][j]))
                    entries[idx][i][j].configure(bg='white', state='readonly')
    ...
    def select_number(number):
        ...
        for i in range(9):
            for j in range(9):
                if entries[current_thread][i][j].config('bg')[-1] == 'yellow' and sudoku_puzzles[current_thread][i][j] == 0:
