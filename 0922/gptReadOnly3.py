...
def create_game_interface():
    ...
    def generate_interface(thread_count):
        ...
        for i in range(3):
            for j in range(3):
                ...
                button = tk.Button(number_frame, text=str(button_number), width=8,
                                command=lambda num=button_number, thread_idx=current_thread: select_number(num, thread_idx))
    ...
    def select_number(number, thread_idx):
        for i in range(9):
            for j in range(9):
                if entries[thread_idx][i][j].config('bg')[-1] == 'yellow':
                    entries[thread_idx][i][j].delete(0, tk.END)
                    entries[thread_idx][i][j].insert(0, str(number))
                    # 清除高亮选择
                    for row in range(9):
                        for col in range(9):
                            entries[thread_idx][row][col].config(bg='white')
    ...
    def start_game():
        ...
        # 在主线程中生成数独题目和答案
        generated_puzzles, generated_answers = generate_sudoku_parallel(thread_count, default_difficulty_level)

        # 验证数独题目的唯一解性
        valid_puzzles = validate_puzzles(generated_puzzles)

        if len(valid_puzzles) == thread_count:
            # 所有数独题目都有唯一解
            sudoku_puzzles = generated_puzzles
            sudoku_answers = generated_answers

            # 生成界面
            generate_interface(thread_count)
        else:
            messagebox.showerror("错误", "生成数独题目失败，请重试。")
