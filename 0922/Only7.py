...
class SudokuGUI:
    def __init__(self, master, sudoku_list):
        ...
        # 添加开始游戏的按钮
        start_button = tk.Button(self.master, text="开始游戏", command=self.start_game)
        start_button.pack()

        # 添加选择题目的按钮
        select_button = tk.Button(self.master, text="选择题目", command=self.select_puzzle)
        select_button.pack()
